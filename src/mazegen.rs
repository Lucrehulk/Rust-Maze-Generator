...
impl MazeManager {
    ...
    fn square_quantize_check_next_square_magnitude(&self, coord: [usize; 2], depth: usize) -> bool {
        if coord[0] + depth == self.width || coord[1] + depth == self.height {
            return false;
        }
        let mut square_satisfied = true;
        let mut y_iter = Vec::new();
        for y in coord[1]..=coord[1] + depth {
            y_iter.push([coord[0] + depth, y]);
        }
        for coord in y_iter {
            if !self.grid[coord[1]][coord[0]] {
                square_satisfied = false;
                break;
            }
        }
        if square_satisfied {
    ...
    pub fn square_quantize_maze(&self) -> Vec<[usize; 3]> {
        ...
        let mut square_jumps: HashMap<[usize; 2], usize> = HashMap::new();
        while y < self.height {
            if square_jumps.contains_key(&[x, y]) {
                x = *square_jumps.get(&[x, y]).unwrap();
                if x == self.width {
                    x = 0;
                    y += 1;
                }
            ...
            if self.grid[y][x] {
                ...
                while self.square_quantize_check_next_square_magnitude([x, y], square_magnitude) {
                    square_magnitude += 1;
                }
                completed_squares.push([x, y, square_magnitude]);
                for y_coord in y..y + square_magnitude {
                    square_jumps.insert([x, y_coord], x + square_magnitude);
                }
                x = x + square_magnitude;
                if x == self.width {
                    x = 0;
                    y += 1;
                }
