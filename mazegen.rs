...
struct MazeManager {
    grid: Vec<Vec<bool>>,
    true_tiles: HashSet<[usize; 2]>,
    false_tiles: HashSet<[usize; 2]>,
    width: usize,
    height: usize,
    center_x: usize,
    center_y: usize,
...
impl MazeManager {
    ...
    fn set_tile(&mut self, x: usize, y: usize, set_to: bool) {
    ...
    fn instantiate_basic_erosion_maze(width: usize, height: usize, weight: f32, rng: &mut ThreadRng) -> MazeManager {
        let mut map = MazeManager::instantiate_map(width, height, false);
        map.erode_tiles(false, false, true, false, false, weight, 7, rng);
        map
    }

    fn instantiate_random_tunnel_maze(width: usize, height: usize, weight: f32, directions_weighted_by_coordinates: bool, max_direction_factor: usize, rng: &mut ThreadRng) -> MazeManager {
    ...
    fn instantiate_tunnel_maze(width: usize, height: usize, fill_holes: bool, weight: f32, original_parent_tunnel_distance: f32, min_min_tunnel_distance: f32, max_min_tunnel_distance: f32, rng: &mut ThreadRng) -> MazeManager {
        let mut map = MazeManager::instantiate_map(width, height, true);
    ...
    fn instantiate_branch_maze(width: usize, height: usize, fill_holes: bool, min_branch_length: usize, max_branch_length: usize, weight: f32, branch_direction_sway_factor: f32, rng: &mut ThreadRng) -> MazeManager {
        let mut map = MazeManager::instantiate_map(width, height, true);
    ...
    fn instantiate_branched_erosion_maze(width: usize, height: usize, weight: f32, horizontal_step: usize, vertical_step: usize, rng: &mut ThreadRng) -> MazeManager {
        let mut map = MazeManager::instantiate_map(width, height, true);
